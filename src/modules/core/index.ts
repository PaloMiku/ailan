...
export default class extends Module {
	...
	private transferBegin(msg: Message): boolean  {
		if (!msg.text) return false;
		if (!msg.includes(['交接', '搬家'])) return false;
	...
	private setName(msg: Message): boolean  {
		if (!msg.text) return false;
		if (!msg.text.includes('请称呼为')) return false;
		if (msg.text.startsWith('请称呼为')) return false;
	...
	private async contextHook(key: any, msg: Message, data: any) {
		...
		if (msg.text.includes('好的')) {
			msg.friend.updateName(data.name + '桑');
			done();
		} else if (msg.text.includes('不')) {
